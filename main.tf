...
# ──────────────────────────────────────────────────────────────────────────
# VCS-Driven main.tf
# NOTE: No cloud {} block! TFC connects via its VCS integration UI setting.
# ──────────────────────────────────────────────────────────────────────────
...
data "aws_ami" "amazon_linux" {
  ...
  filter {
    name   = "name"
    values = ["al2023-ami-*-x86_64"] # Amazon Linux 2023 (preferablly  over AL2)
  }
}
...
# Security Group — SSH restricted by var.allowed_ssh_cidr
resource "aws_security_group" "ssh" {
  name        = "${var.environment}-allow-ssh"
  description = "Allow SSH inbound from approved CIDR"
  vpc_id      = aws_vpc.main.id
...
resource "aws_instance" "public_instance" {
  ...
  key_name               = var.key_name   #updated Key value to dev_key1
